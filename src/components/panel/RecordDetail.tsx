  ...
  const mdComponents = useMemo(
    () => ({
      h1: ({ children }: { children?: ReactNode }) => (
        <h1 className="text-xl font-semibold mt-6 mb-3 text-slate-100 scroll-mt-4">
          {children}
        </h1>
      ),
      h2: ({ children }: { children?: ReactNode }) => (
        <h2 className="text-lg font-semibold mt-5 mb-2 text-slate-100 scroll-mt-4">
          {children}
        </h2>
      ),
      h3: ({ children }: { children?: ReactNode }) => (
        <h3 className="text-base font-semibold mt-4 mb-2 text-slate-200 scroll-mt-4">
          {children}
        </h3>
      ),
      p: ({ children }: { children?: ReactNode }) => (
        <p className="text-sm leading-6 text-slate-300 my-2">{children}</p>
      ),
      ul: ({ children }: { children?: ReactNode }) => (
        <ul className="list-disc pl-5 space-y-1 text-sm text-slate-300 my-2">{children}</ul>
      ),
      ol: ({ children }: { children?: ReactNode }) => (
        <ol className="list-decimal pl-5 space-y-1 text-sm text-slate-300 my-2">{children}</ol>
      ),
      li: ({ children }: { children?: ReactNode }) => (
        <li className="text-sm leading-6 text-slate-300">{children}</li>
      ),
      ...
      blockquote: ({ children }: { children?: ReactNode }) => (
        <blockquote className="border-l-2 border-emerald-400/30 bg-white/[2%] pl-4 py-2 my-3 text-slate-400 italic text-sm">
          {children}
        </blockquote>
      ),
      code: ({
        ...
        return (
          <code className="rounded bg-slate-800/80 px-1.5 py-0.5 text-[0.85em] text-emerald-300">
            {children}
          </code>
        );
      },
      pre: ({ children }: { children?: ReactNode }) => (
        <pre className="rounded-xl border border-white/[8%] bg-slate-900/80 px-4 py-3 overflow-x-auto text-[13px] my-3">
          {children}
        </pre>
      ),
      ...
      th: ({ children }: { children?: ReactNode }) => (
        <th className="border border-white/10 px-3 py-1.5 text-left text-slate-300 bg-white/5">
          {children}
        </th>
      ),
      td: ({ children }: { children?: ReactNode }) => (
        <td className="border border-white/10 px-3 py-1.5 text-slate-400">{children}</td>
      ),
      ...
      img: ({ src, alt }: { src?: string; alt?: string }) => (
        <img
          src={src}
          alt={alt}
          className="max-h-96 w-full rounded-xl border border-white/5 object-contain my-3"
        />
      ),
    }),
    [],
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1">
        {editingContent ? (
          ...
          <div className="flex min-w-0 flex-1">
            <textarea
              ...
              className="flex-1 resize-none border-r border-white/5 bg-slate-900/60
                px-5 py-4 text-sm leading-6 text-slate-100 outline-none
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-5">
              {contentDraft.trim() ? (
                ...
                <p className="text-sm italic text-slate-500">实时预览…</p>
              )}
            </div>
          </div>
        ) : (
            ...
            <div className="space-y-5 p-5">
                ...
                {editingTitle ? (
                  <input
                    ...
                    className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-3 py-2
                      text-base font-medium text-slate-100 outline-none transition
              ...
              <section>
                ...
                {record.content ? (
                  ...
                  >
                    <div className="markdown-body transition group-hover:text-slate-200" ref={markdownContainerRef}>
                      <ReactMarkdown remarkPlugins={[remarkGfm]} components={mdComponents}>
                        {record.content}
                      </ReactMarkdown>
                    </div>
                    <span className="mt-1 block text-[10px] text-slate-600 opacity-0 transition group-hover:opacity-100">
                      点击编辑
                    </span>
                  </div>
                ) : (
        ...
        {/* ── TOC right rail ── */}
        {toc.length > 0 && (
          <aside className="w-[180px] shrink-0 overflow-y-auto border-l border-white/5 bg-slate-950/30 px-3 py-4">
            <p className="mb-2 text-[10px] font-medium uppercase tracking-[0.2em] text-slate-500">
              目录
            </p>
            <nav className="space-y-0.5">
              {toc.map((entry, i) => (
                <button
                  ...
                  className="block w-full truncate text-left text-[11px] leading-5 text-slate-400 transition hover:text-emerald-300"
                >
                  {entry.text}
                </button>
              ))}
        ...
        <div className="shrink-0 border-t border-white/5 px-5 py-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-emerald-400/15 px-4 py-1.5
                text-xs font-medium text-emerald-300 transition hover:bg-emerald-400/25"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5
                text-xs font-medium text-slate-400 transition hover:bg-white/5 hover:text-slate-200"
            >
              取消
            </button>
